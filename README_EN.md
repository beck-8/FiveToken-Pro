...
- **Debug build** (`--debug`, and `--release` without a keystore): signed with
  ...
  The **package name is always `io.fivetokenpro.fil`** for everyone, but each
...
- **Release build** (shareable, updatable): generate a release keystore, keep it
  **private** (never commit it), and configure `key.properties`:
  ...
  Sign **all** official releases with the **same** keystore so users can update
  without reinstalling (and without losing their wallets). Both
  `android/key.properties` and the keystore file are gitignored.
